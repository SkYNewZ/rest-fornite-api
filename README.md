...
## Install
```bash
$ git clone https://github.com/SkYNewZ/fortnite-api
$ cd fortnite-api && npm install
$ cd .. && npm link fortnite-api
```
...
$ docker run -it --rm \     
  ...
  fortnite-api:latest
```
This will prompt if the user if `example.js` exit, and confirm your credentilas
Example :
```
~/Docker/fortnite-api   develop ●  docker run -it --rm \     
  -e LOGIN_EMAIL=XXXX \
  -e LOGIN_PASSWORD=XXXX \
  -e OAUTH_EPIC_LAUNCHER=XXXX \
  -e OAUTH_FORTNITE=XXXX \
  fortnite-api:latest

  Fortnite-API - Initialisation OK
  Fortnite-API - Credentials Params OK
  { id: '7c09fe60327b48628b5aa8a56d226eb9',
  displayName: 'Xewtwo' }
```
